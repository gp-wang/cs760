...
public class DataSet {
	...
	//attributes
	public int attrCnt = 0; // does NOT count label in
	...
	public int getCount_label(int label) {

		int total = 0;
		for (Entry<Integer, Instance> ent : instances.entrySet()) {
			Instance ins = ent.getValue();
			if(ins.label == label) {
				++ total;
			}
	...
	public double getCount_features_label(int label,
			Map<Integer, String> feature_val_map) {


		int cnt = 0;
		for(Entry<Integer, Instance> ent : instances.entrySet()) {
			Instance example = ent.getValue();

			if (example.label == label){
				boolean match = true;      

				for(Entry<Integer, String> ent1 : feature_val_map.entrySet()) {
					int feature = ent1.getKey();

					String featureValue = ent1.getValue();
					String ex_feat_value = example.getFeatureValue(feature, this);
					if (!ex_feat_value.equals(featureValue)) {
						match = false;
						break;
					}
				}
